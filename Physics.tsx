...
export const addPipesAtLocation = (x, world, entities) => {
    ...
    let pipe1Top = Metter.Bodies.rectangle(
        x,
    ...
    let pipe2Top = Metter.Bodies.rectangle(
        x,
    ...
    Metter.World.add(world, [pipe1, pipe1Top, pipe2, pipe2Top]);
...
const Physics = (entities, {touches, time, dispatch}) => {
    ...
    Object.keys(entities).forEach(key => {
        if (key.indexOf('pipe') === 0  && entities.hasOwnProperty(key)){
            Metter.Body.translate(entities[key].body, {x: -2, y: 0});
            ...
            if (entities[key].body.position.x <= -1 * Constants.MAX_WIDTH /2 ) {
                Metter.Body.setPosition(entities[key].body, {x: Constants.MAX_WIDTH + ( Constants.MAX_WIDTH / 2 ) , y: entities[key].body.position.y});
            } else {
                Metter.Body.translate(entities[key].body, {x: -2, y: 0});
            }
